...
public class BluetoothClient extends BlePduUtil {

    private static final String TAG = "bleClient";

    private static final String PREFIX = "ffffff";
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                if (service != null) {
                    ...
                    mBluetoothGatt.setCharacteristicNotification(service.getCharacteristic(UUID.fromString(mUuid)), true);
                }
            }
        }
        ...
        public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                if (!isBleRange) {
                    Log.d(TAG, "Bluetooth 未开启蓝牙围栏 停止蓝牙扫描2");
                    stopScan();
                }
            } else {
        ...
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
            ...
            BluetoothGattService service = mBluetoothGatt.getService(UUID.fromString(mUuid));
            if (service != null) {
                boolean b = mBluetoothGatt.readCharacteristic(service.getCharacteristic(UUID.fromString(mUuid)));
    ...
    private ScanCallback scanCallback = new ScanCallback() {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
            ...
            if (ids != null) {
                for (ParcelUuid parcelUuid : ids) {
                    Log.d(TAG, "Scan Device:" + parcelUuid.toString()
                            + "  mTargetMac:" + mTargetMac);

                    if (mTargetMac != null
                            && parcelUuid.toString().startsWith(mTargetMac, 8)) {
                        mUuid = parcelUuid.toString();
                        connectDevice(device);
                        break;
                    }
                    if (isBleRange && parcelUuid.toString().startsWith(PREFIX)) {
                        String mac = initMac(parcelUuid.toString());
                        mBluetoothClientCallback.onScanResult(mac);
                        Log.d(TAG, "find CooCaa Ble Device:" + mac);
                    }
                }
            }
        ...
        public void onScanFailed(int errorCode) {
            ...
            Log.d(TAG, "onScanFailed isScan:" + isScan);
    ...
    public BluetoothClient(Context context, BluetoothClientCallback callback,
                           Handler processHandler, boolean openBleRange) {
        mContext = context;
    ...
    public void startScan(String mac) {
        ...
        if (mac != null && mac.length() == 12) {
            mac = mac.toLowerCase();
            mTargetMac = "-" + mac.substring(0, 4) + "-" + mac.substring(4, 8) + "-" + mac.substring(8) + "-";
            mUuid = null;
            ...
            mProcessHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    Log.d(TAG, "startScan(mac) target mac:" + mTargetMac);
    ...
    public void startScan() {
        ...
        mProcessHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                Log.d(TAG, "startScan() target mac:" + mTargetMac);
                mBluetoothAdapter.getBluetoothLeScanner().startScan(scanCallback);
                updateState(BluetoothClientCallback.DeviceState.SCANING);
            }
        });
    ...
    private void sendMsgByThread(final String msg, final byte type) {
        if (mBluetoothGatt == null || mUuid == null) {
            Log.d(TAG, "sendMsgByThread error!!!");
            return;
        }

        BluetoothGattService gattService = mBluetoothGatt.getService(UUID.fromString(mUuid));
        if (gattService != null) {

            final BluetoothGattCharacteristic gattCharacteristic = gattService.getCharacteristic(UUID.fromString(mUuid));
    ...
    public boolean isConnected(String mac) {
        ...
        if (targetMac.equals(mTargetMac) && bleStatus == BLE_STATUS.CONNECTED) {
            return true;
        }
        return false;
    }
    ...
    public void disConnect() {
        ...
        if (!isBleRange) {
            Log.d(TAG, "Bluetooth 未开启蓝牙围栏 停止蓝牙扫描1");
            stopScan();
        }
    }
    ...
    private boolean isSupport() {
        ...
        if (Build.VERSION.SDK_INT >= 29) { //判断是否为android6.0系统版本，如果是，需要动态添加权限
            if (ContextCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_BACKGROUND_LOCATION) != PackageManager.PERMISSION_GRANTED
                /*|| ContextCompat.checkSelfPermission(mContext, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED*/) {// 没有权限，申请权限。
    ...
    private static class UIHandler extends Handler {
        WeakReference<BluetoothClient> weakReference;

        public UIHandler(BluetoothClient bleClient) {
        ...
        public void handleMessage(Message msg) {
            BluetoothClient bleClient = weakReference.get();
            switch (msg.what) {
