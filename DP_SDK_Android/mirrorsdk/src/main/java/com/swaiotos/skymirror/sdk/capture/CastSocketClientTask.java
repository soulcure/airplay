...
//    private DataInputStream in =null;// 输入流
    ...
    private int THREAD_POOL_NUM = 10;
    int mframeIdx = 0;
    ...
    public CastSocketClientTask(String address, final int port, int threadnum) {
        Log.d(TAG, "create SocketClientTask ");
        this.mAddress = address;
        this.port = port;
        DataSocketClientThread st =  new DataSocketClientThread();
        THREAD_POOL_NUM =threadnum;
        mframeIdx = 0;
        new Thread(st).start();
    }
...
//        Log.d(TAG,"sendData with socket number"+socket_num);
        ...
        Log.d(TAG,"sendData:"+port+": with max len:"+max_slice_size+",socket_num:"+socket_num);
        int pos = 0;
//        try {
            for (int i = 0; i < socket_num; i++) {
                ...
                Log.d(TAG,"sendData:"+port+": with sendSize:"+sendSize+"width frameIdx = "+ (mframeIdx&0xff));
                ...
                sdata[5] = (byte) (i);
                sdata[6] = (byte) (mframeIdx & 0xff);//frameIndex
                sdata[7] = 0;//reserved
