...
public enum SizeConverter {
    ...
    public static void main(String[] args) {
        System.out.println(SizeConverter.KBTrim.convert(419562f));
    }


    public static String countConvert(long count) {
        if (count < 0 || count == 0) {
            return "";
        } else if (count > 0 && count < 10000) {
            return count + "次安装";
        } else if (count > 10000 && count < 100000000) {
            return String.format("%.1f", count / 10000f) + "万次安装";
        } else {
            return String.format("%.1f", count / 100000000f) + "亿次安装";
        }
    }
