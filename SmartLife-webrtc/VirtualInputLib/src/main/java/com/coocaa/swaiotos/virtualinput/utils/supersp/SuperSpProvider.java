 ...
 * @Author: yuzhan
 */
public class SuperSpProvider extends ContentProvider {

    private Map<String, MethodProcess> processerMap = new ArrayMap<>();

    @Override
    public boolean onCreate() {
        processerMap.put(SuperSpUtil.METHOD_QUERY_VALUE, methodQueryValues);
        processerMap.put(SuperSpUtil.METHOD_CONTAIN_KEY, methodContainKey);
        processerMap.put(SuperSpUtil.METHOD_EIDIT_VALUE, methodEditor);
        processerMap.put(SuperSpUtil.METHOD_QUERY_PID, methodQueryPid);
        return true;
    }
    ...
    private MethodProcess methodQueryPid = new MethodProcess() {
        ...
        public Bundle process(@Nullable String arg, @Nullable Bundle extras) {
            ...
            bundle.putInt(SuperSpUtil.KEY_VALUES, android.os.Process.myPid());
            return bundle;
        }
    };
    ...
    private MethodProcess methodContainKey = new MethodProcess() {
        ...
        public Bundle process(@Nullable String arg, @Nullable Bundle extras) {
            ...
            String key = extras.getString(OpEntry.KEY_KEY);
            SharedPreferences preferences = ctx.getSharedPreferences(arg, Context.MODE_PRIVATE);
            extras.putBoolean(SuperSpUtil.KEY_VALUES, preferences.contains(key));
            return extras;
        }
    };
    ...
    private MethodProcess methodEditor = new MethodProcess() {
        ...
        public Bundle process(@Nullable String arg, @Nullable Bundle extras) {
            ...
            SharedPreferences preferences = ctx.getSharedPreferences(arg, Context.MODE_PRIVATE);
            ArrayList<Bundle> ops = extras.getParcelableArrayList(SuperSpUtil.KEY_VALUES);
            if (ops == null) {
                ops = new ArrayList<>();
            }
            ...
            if (applyOrCommit == OpEntry.OP_TYPE_APPLY) {
                ...
                Bundle bundle = new Bundle();
                bundle.putBoolean(SuperSpUtil.KEY_VALUES, res);
                return bundle;
            } else {
