package com.coocaa.swaiotos.virtualinput.utils.supersp;
 ...
 * @Author: yuzhan
 */
public class SuperSpProxy implements SharedPreferences {
    private static Map<String, SuperSpProxy> sharedPreferenceProxyMap;
    ...
    private SuperSpProxy(Context ctx, String name) {
    ...
    public boolean contains(String key) {
        ...
        try {
            Bundle res = ctx.getContentResolver().call(SuperSpUtil.URI
                    , SuperSpUtil.METHOD_CONTAIN_KEY
                    , preferName
                    , input);
            return res.getBoolean(SuperSpUtil.KEY_VALUES);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    private OpEntry getResult(@NonNull OpEntry input) {
        try {
            Bundle res = ctx.getContentResolver().call(SuperSpUtil.URI
                    , SuperSpUtil.METHOD_QUERY_VALUE
                    , preferName
                    , input.getBundle());
    ...
    public class EditorImpl implements Editor {
        ...
        public boolean commit() {
            Bundle input = new Bundle();
            input.putParcelableArrayList(SuperSpUtil.KEY_VALUES, convertBundleList());
            input.putInt(OpEntry.KEY_OP_TYPE, OpEntry.OP_TYPE_COMMIT);
            Bundle res = null;
            try {
                res = ctx.getContentResolver().call(SuperSpUtil.URI, SuperSpUtil.METHOD_EIDIT_VALUE, preferName, input);
            } catch (Exception e) {
                e.printStackTrace();
            }
            if (res == null) {
            ...
            return res.getBoolean(SuperSpUtil.KEY_VALUES, false);
        }

        @Override
        public void apply() {
            Bundle intput = new Bundle();
            intput.putParcelableArrayList(SuperSpUtil.KEY_VALUES, convertBundleList());
            intput.putInt(OpEntry.KEY_OP_TYPE, OpEntry.OP_TYPE_APPLY);
            try {
                ctx.getContentResolver().call(SuperSpUtil.URI, SuperSpUtil.METHOD_EIDIT_VALUE, preferName, intput);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public static SharedPreferences getSharedPreferences(@NonNull Context ctx, String preferName) {
        ...
        if (processFlag.get() == 0) {
            Bundle bundle = ctx.getContentResolver().call(SuperSpUtil.URI, SuperSpUtil.METHOD_QUERY_PID, "", null);
            int pid = 0;
            if (bundle != null) {
                pid = bundle.getInt(SuperSpUtil.KEY_VALUES);
            }
    ...
    private static SharedPreferences getFromRemoteProcess(@NonNull Context ctx, String preferName) {
        synchronized (SuperSpProxy.class) {
            if (sharedPreferenceProxyMap == null) {
                sharedPreferenceProxyMap = new ArrayMap<>();
            }
            SuperSpProxy preferenceProxy = sharedPreferenceProxyMap.get(preferName);
            if (preferenceProxy == null) {
                preferenceProxy = new SuperSpProxy(ctx.getApplicationContext(), preferName);
