...
public class StatusBarHelper {
    ...
    @TargetApi(19)
    public static void transparencyBar(Window window, int color) {
    ...
    public static void translucent(Window window, @ColorInt int colorOn5x) {
        ...
        if (DeviceHelper.isFlymeLowerThan8() || (DeviceHelper.isMIUI() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M)) {
            window.setFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS,
                    WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            return;
        }
...
//            }
    ...
    public static boolean setStatusBarLightMode(Window window) {
        ...
        if (mStatuBarType != STATUSBAR_TYPE_DEFAULT) {
            return setStatusBarLightMode(window, mStatuBarType);
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
    ...
    private static boolean setStatusBarLightMode(Window window, @StatusBarType int type) {
        if (type == STATUSBAR_TYPE_MIUI) {
    ...
    private static boolean Android6SetStatusBarLightMode(Window window, boolean light) {
        ...
        decorView.setSystemUiVisibility(systemUi);
        if (swaiotos.runtime.base.DeviceHelper.isMIUIV9()) {
            // MIUI 9 低于 6.0 版本依旧只能回退到以前的方案
            // https://github.com/Tencent/QMUI_Android/issues/160
            MIUISetStatusBarLightMode(window, light);
        }
