...
public class AvcDecoder2 {
    private static final String TAG = "AvcDecoder2";
    /**
     * YYYYYYYY UU VV
     */
...
//    private ExecutorService executorPools = Executors.newCachedThreadPool();
    ...
    public void setDecoderParams(String mp4Path, int fileType) throws IOException {
        Log.d(TAG,"setDecoderParams,mp4Path:"+mp4Path+",fileType:"+fileType);
...
//            }
                ...
                if (doRender) {
                    ...
                    if (outputImageFileType != -1 && outputImageFileType == FILE_TypeI420) {

                        byte[] data = getDataFromImage(image, COLOR_FormatI420);
                        Log.d(TAG,"getNextFrame,data liength:"+data.length);
                        ...
                        Log.d(TAG,"getNextFrame,Y:"+dataY.capacity()+",U:"+dataU.capacity()+",V:"+dataV.capacity());
                        int totolneed = dataY.capacity()+dataU.capacity()+dataV.capacity();
                        Log.d(TAG,"getNextFrame Y+U+V:"+totolneed);
                        int pos = 0;
                        Log.d(TAG,"getNextFrame,pos:"+pos);
                        dataY.put(data,pos,dataY.capacity());
                        pos += dataY.capacity();
                        Log.d(TAG,"getNextFrame,pos:"+pos);
                        dataU.put(data,pos,dataU.capacity());
                        pos += dataU.capacity();
                        Log.d(TAG,"getNextFrame,pos:"+pos);
                        dataV.put(data,pos,dataV.capacity());
                        pos += dataV.capacity();


                        videoFrame = new VideoFrame(i420buffer,0,captureTimeNs);
...
//            decodeProgressListener.publishProgress(-1);
            if (decoder != null) {
                decoder.stop();
                decoder.release();
                decoder = null;
