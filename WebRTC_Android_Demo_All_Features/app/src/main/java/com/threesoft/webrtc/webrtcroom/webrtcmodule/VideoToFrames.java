...
public class VideoToFrames implements Runnable {
    ...
    public interface Callback {
        ...
        void onDecodeFrame(VideoFrame frame);
    }
    ...
    public void setFormate(OutputImageFormat imageFormat)  {
    ...
    public void videoDecode(String videoFilePath) throws IOException {
        Log.d(TAG,"videoDecode,videoFilePath:"+videoFilePath);
    ...
    private void decodeFramesToImage(MediaCodec decoder, MediaExtractor extractor, MediaFormat mediaFormat) {
        ...
        Log.d(TAG,"decodeFramesToImage,formate:"+mediaFormat);
        decoder.configure(mediaFormat, null, null, 0);
        decoder.start();
        final int width = mediaFormat.getInteger(MediaFormat.KEY_WIDTH);
        final int height = mediaFormat.getInteger(MediaFormat.KEY_HEIGHT);
        Log.d(TAG,"decodeFramesToImage,width:"+width+",height:"+height);
        ...
        while (!sawOutputEOS && !stopDecode) {
            ...
            if(outputBufferId == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED){
                Log.d("decodeFramesToImage", "New format " + decoder.getOutputFormat());

            }
            ByteBuffer[] byteBuffer2 = decoder.getOutputBuffers();
...
//            if (outputBufferId >= 0) {
                ...
                if (doRender) {
                    outputFrameCount++;
                    ByteBuffer byteBuffer = decoder.getOutputBuffer(outputBufferId);
                    ...
                    byteBuffer.get(dataY.array(),0,dataY.array().length);
                    pos += dataY.array().length;
                    byteBuffer.get(dataU.array(),pos,dataU.array().length);
                    pos += dataU.array().length;
                    byteBuffer.get(dataV.array(),pos,dataV.array().length);
                    VideoFrame videoFrame = new VideoFrame(i420buffer,0,captureTimeNs);
                    Log.d(TAG,"decodeFramesToImage,outputBufferId:"+outputBufferId+",byteBufferSize:"+byteBuffer.array().length);
                    if(callback != null){
                        callback.onDecodeFrame(videoFrame);
                    }
